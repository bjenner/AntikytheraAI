...
module a2_ring(od = 14, id = 8, th = 2) {
    difference() {
...
module a4_block(w = 12, d = 8, h = 24, bore_d = 8) {
    difference() {
...
module a5_ring(od = 12, id = 8, th = 2) { a2_ring(od = od, id = id, th = th); }

module a6_bush(od = 12, id = 8, h = 17) {
    difference() {
        union() {
...
module drw001_sheet5() {
    ...
    color([0.90, 0.90, 0.90]) translate([130, 165, 0]) linear_extrude(height = th) projection(cut = true) a2_ring();
    color("black") translate([130, 165, 0.01]) linear_extrude(height = th) outline2d(0.34) projection(cut = true) a2_ring();
    color("black") translate([150, 216, 0]) linear_extrude(height = th) label("a2", sz = 3.1, bold = true);

    color([0.90, 0.90, 0.90]) translate([206, 164, 0]) linear_extrude(height = th) projection(cut = true) a3_pin();
    color("black") translate([206, 164, 0.01]) linear_extrude(height = th) outline2d(0.34) projection(cut = true) a3_pin();
    color("black") translate([214, 227, 0]) linear_extrude(height = th) label("a3", sz = 3.1, bold = true);

    color([0.90, 0.90, 0.90]) translate([310, 160, 0]) linear_extrude(height = th) projection(cut = true) a4_block();
    color("black") translate([310, 160, 0.01]) linear_extrude(height = th) outline2d(0.34) projection(cut = true) a4_block();
    color("black") translate([324, 198, 0]) linear_extrude(height = th) label("a4", sz = 3.1, bold = true);

    color([0.90, 0.90, 0.90]) translate([130, 72, 0]) linear_extrude(height = th) projection(cut = true) a5_ring();
    color("black") translate([130, 72, 0.01]) linear_extrude(height = th) outline2d(0.34) projection(cut = true) a5_ring();
    color("black") translate([138, 95, 0]) linear_extrude(height = th) label("a5", sz = 3.1, bold = true);

    color([0.90, 0.90, 0.90]) translate([311, 73, 0]) linear_extrude(height = th) projection(cut = true) a6_bush();
    color("black") translate([311, 73, 0.01]) linear_extrude(height = th) outline2d(0.34) projection(cut = true) a6_bush();
    color("black") translate([332, 126, 0]) linear_extrude(height = th) label("a6", sz = 3.1, bold = true);

    color([0.90, 0.90, 0.90]) translate([226, 72, 0]) linear_extrude(height = th) projection(cut = true) a12_link();
    color("black") translate([226, 72, 0.01]) linear_extrude(height = th) outline2d(0.34) projection(cut = true) a12_link();
    ...
    color([0.28, 0.23, 0.20]) translate([129, 234, 18]) rotate([68, 0, 25]) scale([1.0, 1.0, 1.0]) a2_ring();
    color([0.55, 0.46, 0.40]) translate([206, 226, 18]) rotate([0, 0, 0]) scale([1.0, 1.0, 1.0]) a3_pin();
    color([0.45, 0.30, 0.18]) translate([306, 198, 18]) rotate([0, 0, 0]) scale([1.0, 1.0, 1.0]) a4_block();
    color([0.45, 0.30, 0.18]) translate([126, 92, 18]) rotate([68, 0, 25]) scale([0.95, 0.95, 0.95]) a5_ring();
    color([0.38, 0.30, 0.22]) translate([329, 93, 18]) rotate([0, 0, 0]) scale([1.0, 1.0, 1.0]) a6_bush();
    color([0.30, 0.24, 0.20]) translate([228, 112, 18]) rotate([70, 0, -25]) scale([1.0, 1.0, 1.0]) a12_link();
