...
//
// Part E1: thin spur gear shown in the DRW-004 gear set detail.
//
...
function e1_polar(r, a_deg) = [r * cos(a_deg), r * sin(a_deg)];

function e1_tip_land_deg(tip_land_w = 0.28, outer_d = 17.20) =
    2 * asin(min(1, tip_land_w / outer_d));

function e1_root_land_deg(
    tip_land_w = 0.28,
    outer_d = 17.20
) = e1_tip_land_deg(
...
module e1_tooth_gap_2d(
    outer_r = 8.60,
    root_r = 7.48,
    pitch_deg = 360 / 32,
    tip_land_deg = e1_tip_land_deg(),
    root_land_deg = e1_root_land_deg(),
    shoulder_r = 8.18,
    shoulder_deg = 2.30,
    throat_r = 7.60,
    throat_deg = 1.42,
    ...
    polygon(points = [
        e1_polar(outer_r + overshoot_r, -gap_half_deg),
        e1_polar(shoulder_r, -shoulder_deg),
        e1_polar(throat_r, -throat_deg),
        e1_polar(root_r, -root_deg),
        e1_polar(root_r, root_deg),
        e1_polar(throat_r, throat_deg),
        e1_polar(shoulder_r, shoulder_deg),
        e1_polar(outer_r + overshoot_r, gap_half_deg)
...
module e1_center_bore_2d(
    main_r = 3.0,
    upper_relief_r = 1.0,
    lower_relief_r = 1.0,
    upper_relief_offset_y = 3.0,
    lower_relief_offset_y = -3.0
) {
    union() {
        circle(r = main_r, $fn = 72);
...
module part_e1(
    ...
    tooth_cut_angle_deg = 90,
    bore_main_r = 3.0,
    bore_upper_relief_r = 1.0,
    bore_lower_relief_r = 1.0,
    bore_upper_relief_offset_y = 3.0,
    bore_lower_relief_offset_y = -3.0
) {
    difference() {
