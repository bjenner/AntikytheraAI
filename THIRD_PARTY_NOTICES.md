# Third-Party Notices

This repository includes third-party source material used for analysis and
traceability of the Antikythera project.

## Antikythera Manual and Extracted Sections

- Work: `Antikyther Mechanism Revision 8.1 Operating-Maintanace-Assembly-Dissasambly and Manufacturing Manual Rev 1.1`
- Location in this repo:
  - `docs/sources/Antikythera Mechanism/Antikyther Mechanism Revision 8.1 Operating-Maintanace-Assembly-Dissasambly and Manufacturing Manual Rev 1.1.pdf`
  - `docs/sources/Antikythera Mechanism/manual_sec*.pdf` (extracted page-range sections)
- Copyright notice in source:
  - "Copyright Notice"
  - "Except where noted, © the author; distributed under the terms of the Creative Commons Attribution License."
  - "Creative Commons - Attribution 3.0 Unported - CC BY 3.0"
- License:
  - Creative Commons Attribution 3.0 Unported (CC BY 3.0)
  - https://creativecommons.org/licenses/by/3.0/

### Change/Adaptation Note

Page-range section PDFs (`manual_sec*.pdf`) were extracted from the original
manual for focused review and indexing.

### Important Exception Handling

The source states "Except where noted." Any content within the source that
identifies a different copyright owner or license must be treated under those
stated terms.
